 ...
 *
 * This program is software; you can redistribute it and/or
...
GSM0710_Frame *gsm0710_buffer_get_frame(GSM0710_Buffer *buf) {
  ...
  {
    syslog(LOG_ALERT,"No frame started\n");
    return NULL;
  } 
  ...
  if (gsm0710_buffer_length(buf) >= length_needed) {
    ...
    if (r_crctable[fcs^(*data)] != 0xCF) {
      ...
      syslog(LOG_INFO,"Dropping frame: FCS doesn't match (fcs=%d, *data=%d, r_crctable[fcs^(*data)]=%d, frame data length:%d)\n",(int)fcs, (int)(*data),(int)r_crctable[fcs^(*data)], frame->data_length);
      destroy_frame(frame);
      buf->flag_found = 0;
      buf->dropped_count++;
      buf->readp = data;
...
/* destroys a frame: free the memory.
 *
 * PARAMS:
 * frame: frame pointer to data
 * RETURNS:
 * none
 */
void destroy_frame(GSM0710_Frame *frame) {
#ifdef DEBUG
	syslog(LOG_DEBUG, "destroy_frame");
#endif // DEBUG
	if (frame->data_length > 0)
		free(frame->data);
	free(frame);
