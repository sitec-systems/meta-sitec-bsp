 ...
 *
 * Porting Maj 2007 by MicheleCe to N7 module platforms
 *
 * Modified Jul 2007 by AntonioGi about flow control settings for N7 module platforms
 * Modified Feb 2008 by AntonioGi, send disconnection at the control channel only
 *
 * New Usage:
...
// maximun number of by to read after SIGUSR1 signal
#define MAX_SIGUSR1_DATA   20
...
static int suspend_read_from_virtual_ports = 0;
static int flow_control_on_terminals = 0; //default is no flow control
 ...
 */
static int write_frame(int channel, const char *input, int count, unsigned char type)
{
    ...
    if (suspend_read_from_virtual_ports == 1)
        return 0;
    if(_debug) {
    ...
    if(trace_file)
    {
        ...
        output[count]=0;
        fprintf(trace_file, "To serial port: %s\n", output);
        for(i=0; i<prefix_length; i++)
            fprintf(trace_file, "%02X ",prefix[i]);
        fprintf(trace_file, "\n");
    }
    //------------------------------
    ...
    if (count > 0)
    {
        ...
        if(trace_file!=NULL)
        {
            ...
            for(i=0; i<count; i++)
                fprintf(trace_file, "%02X ", (unsigned char)input[i]);
            fprintf(trace_file, "\n");
        }
...
int ussp_recv_data(unsigned char *buf, int len, int port)
{
    ...
    {
        /* write(serial_fd,"debug_1",7); //N7 debugging */
        last = write_frame(port + 1, buf + written,len - written, UIH);
        /* write(serial_fd,"debug_2",7); //N7 debugging */
        written += last;
        if (last == 0) {
            i++;
        }
...
static int findInBuf(char* buf, int len, char* needle) {
    int i;
    ...
    for (i=0;i<len;i++) {
        if (needle[needleMatchedPos] == buf[i]) {
            needleMatchedPos++;
            if (needle[needleMatchedPos] == '\0')	// Entire needle was found
                return 1;
        } else {
            needleMatchedPos=0;
        }
...
int at_command(int fd, char *cmd, int to)
{
    ...
    if(_debug) {
        syslog(LOG_DEBUG, "writing at_command: %s\n", cmd);
	}
...
int open_pty(char* devname, int idx) {
	...
	if (_debug) {
		syslog(LOG_DEBUG,"symbolic link_name=%s, dev_name=%s\n", symLinkName, devname);
	}
    if (fd != -1) {
        if (symLinkName) {
            ...
            unlink(symLinkName);
			if (_debug) {
				syslog(LOG_DEBUG,"Creating symbolic link %s -> %s ....\n", symLinkName, ptsSlaveName);
			}
        ...
        options.c_iflag &= ~(INLCR | ICRNL | IGNCR | IXON | IXOFF);/* N7 fix, disable XON XOFF explicitely */

        /* N7 extension */
        if(flow_control_on_terminals)
...
int open_serialport(char *dev)
{
    ...
    if (fd != -1)
    {
        ...
        options.c_iflag |= IGNPAR; /* N7 ignore framing and parity errors */
...
static void print_frame(GSM0710_Frame * frame)
{
    if(_debug) {
        syslog(LOG_DEBUG, "is in %s\n" , __FUNCTION__);
                ...
                case C_MSC:
                    if (i + 1 < frame->data_length)
                    {
                        ...
                        if ((signals & S_FC) == S_FC)
                        {
                            ...
                            suspend_read_from_virtual_ports = 1;
                            //for (i = 0; i < numOfPorts; i++)
                            //	tcflow(ussp_fd[i],TCOOFF);
                            tcflow(serial_fd,TCOOFF);
                        }
                        else
                        {
                            int i;
                            ...
                            suspend_read_from_virtual_ports = 0;
                            //for (i = 0; i < numOfPorts; i++)
                            //	tcflow(ussp_fd[i],TCOON);
                            tcflow(serial_fd,TCOON);
                        }
...
static void usage(char *_name)
{
    ...
    fprintf(stderr,"  -H                  : Use hardware flow control on terminals, default is no flow control\n"); /* N7 extension */
...
int extract_frames(GSM0710_Buffer * buf)
{
    ...
    while ((frame = gsm0710_buffer_get_frame(buf)))
    {
        ++framesExtracted;
		if (_debug) {
			syslog(LOG_DEBUG, "Frame extracted, %d\n",framesExtracted);
		}
        if ((FRAME_IS(UI, frame) || FRAME_IS(UIH, frame)))
        {
            ...
            {
                if(_debug) {
                    syslog(LOG_DEBUG,"frame->channel:%d, frame->data_length:%d\n",
                            frame->channel, frame->data_length);
				}
...
/**
 * Daemonize process, this process  create the daemon for the debug
 */
...
void signal_treatment(int param)
{
    switch(param)
    {
        case SIGHUP:			//reread the configuration files
            break;
        case SIGINT:
        case SIGKILL:
        case SIGTERM:
        ...
        case SIGUSR1:
            {
                unsigned char signal[MAX_SIGUSR1_DATA];
                int signal_len = 0;
#if 1
                {
                    unsigned char signal_brake[5] = { 0xe3, 0x07, 0x0B, 0x0D, 0xC3 };
                    memcpy(signal, signal_brake, 5);
                    signal_len = 5;
                }
#else
                printf ("Sending UIH to DLC0. Enter a hexadecimal number ('.' to end): ");
                for(;;)
                {
                    char i;
                    scanf ("%x",&i);
                    if ((i == '.' ) || (signal_len > MAX_SIGUSR1_DATA)) break;
                    signal[signal_len] = i;
                    signal_len ++;
                }
#endif
                write_frame(0, signal, signal_len, UIH);
                syslog(LOG_INFO, "----------> Sending custom signal <-----------------\n");
                sleep(1);
            }
            break;
        case SIGPIPE:
...
int initGeneric()
{
    char mux_command[50]; //"AT+CMUX=0,0,,1509,100,3,150\r\n";
    unsigned char close_mux[2] = { C_CLD | CR, 1 };

    memset(mux_command,0,sizeof(mux_command));

    //liade sprintf(mux_command, "AT+CMUX=0,0,,%d,100,3,150\r\n", max_frame_size);
    sprintf(mux_command, "AT+CMUX=0,0,,%d,100,3,150\r\n", max_frame_size);
    /**
     * Modem Init
     */
...
int openDevicesAndMuxMode() {
    ...
    for (i = 0; i < numOfPorts; i++)
    {
        ...
        if ((ussp_fd[i] = open_pty(ptydev[i], i)) < 0)
        {
            ...
            maxfd = ussp_fd[i];
        cstatus[i].opened = 0;
        cstatus[i].v24_signals = S_FC | S_DV | S_RTR | S_RTC | EA;
    }
    cstatus[i].opened = 0;
    syslog(LOG_INFO,"Open serial port (%s)...\n", serportdev);
...
#define PING_TEST_LEN 6
    ...
    while((opt=getopt(argc,argv,"p:f:hH?dwr:b:P:s:"))>0)
    {
        switch(opt)
        {
            case 'p' :
            ...
            case 'f' :
                ...
                break;
            case 'H' : /* N7 extension */
                flow_control_on_terminals = 1;
            case 'd' :
                _debug = 1;
                trace_file = fopen("mux_trace", "w+");
                break;
            case 's':
    ...
    while (!terminate || terminateCount >= -1)
    {
        ...
        if (sel > 0)
        {
            if (FD_ISSET(serial_fd, &rfds))
            {
                ...
                //read form the real serial port and write data into the buffer and the extract the frames from mux packets (extract_frames)
                if ((size = gsm0710_buffer_free(in_buf)) > 0 && (len = read(serial_fd, buf, min(size, sizeof(buf)))) > 0)
                {
                    ...
                    {
                        int i;
                        fprintf(trace_file, "From serial port\n");
                        for(i=0; i<len; i++)
                            fprintf(trace_file, "%02X ",buf[i]);
                        fprintf(trace_file, "\n\0");
                    }
                    //------------------------------
            ...
            // check virtual ports
            if( 0/*suspend_read_from_virtual_ports == 1*/ )
                syslog(LOG_DEBUG,"Read suspended from virtual ports\n",i,len);
            else
            {
				if (_debug) {
					syslog(LOG_DEBUG,"Read from virtual ports ok\n",i,len);
				}
                //read data form the virtual channels and and encapsulate it in mux packets (ussp_recv_data)
                for (i = 0; i < numOfPorts; i++)
                    if (FD_ISSET(ussp_fd[i], &rfds))
                    {
                        ...
                        {
                            if(_debug) {
                                syslog(LOG_DEBUG,"Non riesco a leggere/scrivere sul canale ptya%d\n",i);
							}
                        }
                        if(len<0)
                        {
        ...
        if (terminate)
        {
...
#if 0 /* N7, send the DISC at the control channel only i.e. disable portion of code */
            if (terminateCount > 0)
            {
            ...
            terminateCount--;
#endif
            /* N7, send the DISC at the control channel only since the DISC at any dlci is not properly done due to an unwanted behaviour at the
               suspend_read_from_virtual_ports variable */
            if (terminateCount > 0)
            {
                syslog(LOG_INFO,"Sending close down request to the multiplexer.\n");
                write_frame(0, close_mux, 2, UIH);
